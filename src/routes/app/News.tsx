...
function News({ debug = false, skip = false }: Props) {
  const [articles, setArticles] = useState<NewsResult>({ status: 'preload', code: 'notCalled', message: 'The news articles have not been loaded.'});
  ...
  useEffect(() => {
    if (!skip && !!API_URL) {
      fetchNews({from: '04/30/24', url: API_URL, sortBy: 'publishedAt', term: 'Trump'})
      .then((fetchNewsResult) => {
